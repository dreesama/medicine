...
public class PdfExportService {
    ...
    public byte[] exportStocksToPdf(List<Stock> stocks) {
        ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();

        try {
            PdfWriter writer = new PdfWriter(byteArrayOutputStream);
            PdfDocument pdfDocument = new PdfDocument(writer);
            pdfDocument.setDefaultPageSize(PageSize.LEGAL.rotate()); // Set to A4 landscape
            ...
            document.add(new Paragraph("Out of Stock Medicines").setBold().setFontSize(18).setTextAlignment(TextAlignment.CENTER).setMarginTop(10));

            document.add(new Paragraph("Total Medicine Out of Stock: " + stocks.size()));
